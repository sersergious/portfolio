...
const mdxComponents = {
    ...
    h1: ({ children, ...props }: any) => (
        <h1 className="text-3xl font-bold mb-6 mt-8 scroll-mt-20 first:mt-0" {...props}>
            {children}
        </h1>
    ),
    ...
    blockquote: ({ children, ...props }: any) => (
        <blockquote className="border-l-4 border-primary pl-4 italic my-6 text-muted-foreground bg-muted/30 py-2 rounded-r" {...props}>
            {children}
        </blockquote>
    ),
    ...
    a: ({ children, href, ...props }: any) => {
        ...
        if (href?.startsWith('/')) {
            return (
                <Link href={href} className="text-primary hover:text-primary/80 underline underline-offset-2" {...props}>
                    {children}
                </Link>
            )
        }
